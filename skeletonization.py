...
def centreOfFirstCase(X):
    if (X[0][0] * X[1][1] - X[0][1] * X[1][0]) != 0 and X[0][0] != 0:
        Yc = -(X[0][0] * X[1][2] - X[1][0] * X[0][2]) / (X[0][0] * X[1][1] - X[0][1] * X[1][0])
        return [-X[0][2] / X[0][0] - X[0][1] * Yc / X[0][0], Yc] 
    else:
        Yc = -((X[0][0] - X[1][0]) * X[1][2] - X[1][0] * (X[0][2] - X[1][2])) / ((X[0][0] - X[1][0]) * X[1][1] - (X[0][1] - X[1][1]) * X[1][0])
        return [-(X[0][2] - X[1][2]) / (X[0][0] - X[1][0]) - (X[0][1] - X[1][1]) * Yc / (X[0][0] - X[1][0]), Yc] 
...
def paramA1(X1, X2, X3):
    if (X2[0] - X3[0]) != 0 :
        ...
        return [[-(X3[2] ** 2 - X2[2] ** 2) / (2 * (X3[0] * X3[2] - X2[0] * X2[2]))],
                [-(X3[1] * X3[2] - X2[1] * X2[2]) / (X3[0] * X3[2] - X2[0] * X2[2])]]
...
def paramB(line, point):
    return [line[0] ** 2 - 1,
            line[0] * line[2] + point[0],
            line[1] ** 2 - 1,
            line[1] * line[2] + point[1],
            line[0] * line[1],
            line[2] ** 2 - point[0] ** 2 - point[1] ** 2]
...
def centreOfSecondCase(A, C):
    if math.fabs((C[1] / C[0]) ** 2 - C[2] / C[0]) < 0.000001:
       if len(A) == 2:
          Yc1 = -C[1] / C[0]
...
def testingCentre(Xc, line):
    ...
    #b = (Xc[0] - line[0][0])*(line[1][1] - line[0][1]) - (Xc[1] - line[0][1])*(line[1][0] - line[0][0])
    if (((math.fabs(a) < 0.005) or (0 <= a <= distPoints(line) ** 2) or (math.fabs(a - distPoints(line) ** 2) < 0.01)) ):#and b>=0): 
...
def intersectionSegments(centre, line, point, segments, points, readyBisector,dist):
    for i in range(len(segments)):
        for k in range(len(points)):
            if (not ([j for j in range(len(readyBisector)) if  segments[i] in readyBisector[j]])
                or not ([j for j in range(len(readyBisector)) if  (points[k] in readyBisector[j][0] and len(readyBisector[j][0])==1 or
                                                                   points[k] in readyBisector[j][1] and len(readyBisector[j][1])==1)])):
                ...
                if testingCentre(projectionC(segments[i],centre),segments[i]):
                    ...
                    if ((distSegment > dist 
                         or math.fabs(distSegment - dist)<0.05) 
                         and (dist_points > dist 
                         or math.fabs(dist_points - dist)<0.05)):
                         0
                    else: return False
    return True
    ...
    for line in lines:
        if ((not line in actBis) and testingCentre(projectionC(line,centre),line) 
            ...
            if (d and 
                ...
                ((line[0][0]< d[0] <line[1][0] or line[0][0]> d[0] >line[1][0])
                and (line[0][1]< d[1] <line[1][1] or line[0][1]> d[1] >line[1][1])
                and (centre[0]< d[0]< actBis[2][0] or centre[0]> d[0]> actBis[2][0])
                and (centre[1]< d[1]< actBis[2][1] or centre[1]> d[1]> actBis[2][1]))):
               return False
    return True
#
...
def addInLists(actBis, tempNodes, activeBisector, readyBisector, skeletNodes, segment,lines):
    ...
    if segment:
        ...
        for nodes in tempNodes:
            if math.fabs(minDistNodes - distPoints([projectionC(segment,actBis[2]),projectionC(segment, nodes[0])])) < 0.05:
                node.append(nodes)
        ...
        for nodes in tempNodes:
            if math.fabs(minDistNodes - distPoints([actBis[2], nodes[0]])) < 0.05:
                node.append(nodes)
                ...
                if  tempNode and (tempNode[0] in X) and not (tempNode[0] in [actBis[2]]):
                        ...
                        if (not [i for i in range(len(skeletNodes)) if  T in [skeletNodes[i][1]]]
                            ...
                            Xc = centreOfSecondCase(paramA1(T, paramOfLine(actBis[0]),paramOfLine(actBis[1])),
                                                                    paramC(paramA1(T, paramOfLine(actBis[0]),
                                                                           ...
                                                                           paramB(paramOfLine(actBis[1]),T)))
                            if Xc:
                                for xc in Xc :
                                    node = []
                                    node.append([testingCentre(xc, actBis[0]),testingCentre(xc, actBis[1])])

                                    if ((node[0][0] and node[0][1]) and comparePoints(node[0][0], actBis[2], 0.5)
                    ...
                    for T in lines:
                        if (not T in actBis 
                            ...
                            if ((node[0][0] and node[0][1] and node[0][2]) and comparePoints(node[0][0], actBis[2], 0.5)
            ...
            if  len(actBis[0]) + len(actBis[1]) == 2: 
                ...
                for T in concaveNodes(X):
                    if (not [T] in actBis and not [i for i in range(len(skeletNodes)) if   T in [skeletNodes[i][1]]]
                        ...
                        if (comparePoints(Xc, actBis[2],0.5) 
                            and testingIntersections(Xc, [], actBis[0][0], lines, concaveNodes(X), readyBisector)
                            and testingCurve(Xc, lines, actBis)):
                ...
                for T in lines: 
                    if (not ([j for j in range(len(readyBisector)) if  T in readyBisector[j]])):
                        Xc = centreOfSecondCase(paramA2(actBis[0][0], actBis[1][0], T), 
                                                paramC(paramA2(actBis[0][0], actBis[1][0], T), paramB(paramOfLine(T),actBis[0][0])))
                        if Xc:
                            for xc in Xc :
                                node = testingCentre(xc, T)
                                if (node and comparePoints(node, actBis[2],0.5)
                            ...
                            if (not T in actBis
                                ...
                                Xc = centreOfSecondCase(paramA1(actBis[i][0], paramOfLine(actBis[1 - i]),paramOfLine(T)),
                                                        paramC(paramA1(actBis[i][0], paramOfLine(actBis[1 - i]),
                                                                       paramOfLine(T)),
                                                               paramB(paramOfLine(T),actBis[i][0])))
                                if Xc :
                                    for xc in Xc :
                                        node = []
    ...
    def effect(self):
        for id, node in self.selected.iteritems():
            if node.tag == inkex.addNS('path','svg'):
                ...
                node.set('d',simplepath.formatPath(AbsPath(Skeletonization(lines(p[0]), X(lines(p[0])), termNode(p[0])))))
